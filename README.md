...
The script `loadtest/k6-chat-ws.js` stresses the WebSocket chat path (senders, receivers, churn, reconnect, typing). It runs inside Docker and needs a running app plus at least one **JWT access token** and one **room id** that the token’s user is a member of.
...
After the run, see `loadtest/POST-RUN-CHECKLIST.md` for what to verify.
